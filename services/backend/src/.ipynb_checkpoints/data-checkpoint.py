...
def load_img_to_tensor(filepath) -> torch.Tensor:
    img = Image.open(filepath)
    return img  # T.functional.pil_to_tensor(img).permute(1, 2, 0)
...
class FlowersDataset(Dataset):
    ...
    def get_raw_img(self, idx: int):
        ...
        return load_img_to_tensor(path)

    def __getitem__(self, idx: int):
        img = self.get_raw_img(idx)
